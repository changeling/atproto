...
__export(src_exports, {
  CompiledRecordSchema: () => CompiledRecordSchema,
  RecordSchemaMalformedError: () => RecordSchemaMalformedError,
  ...
  isValidMethodSchema: () => isValidMethodSchema,
  isValidRecordSchema: () => isValidRecordSchema,
  isValidTokenSchema: () => isValidTokenSchema,
  methodSchema: () => methodSchema,
  methodSchemaBody: () => methodSchemaBody,
  methodSchemaError: () => methodSchemaError,
  methodSchemaParam: () => methodSchemaParam,
...
function isValidRecordSchema(v) {
  return recordSchema.safeParse(v).success;
}
var RecordSchemaMalformedError = class extends Error {
...
var SchemaNotFoundError = class extends Error {
};

// src/record/schema.ts
var import_ajv = __toESM(require_ajv());
var import_ajv_formats = __toESM(require_dist());
var ajv = new import_ajv.default();
(0, import_ajv_formats.default)(ajv);
var CompiledRecordSchema = class {
  constructor(def) {
    this.def = def;
    this.id = def.id;
    try {
...
var schema_default = CompiledRecordSchema;

// src/record/validator.ts
...
var ValidationResultCode = /* @__PURE__ */ ((ValidationResultCode2) => {
  ValidationResultCode2["Full"] = "full";
  ValidationResultCode2["Partial"] = "partial";
  ValidationResultCode2["Incompatible"] = "incompatible";
  ValidationResultCode2["Invalid"] = "invalid";
  return ValidationResultCode2;
...
var ValidationResult = class {
  ...
  get valid() {
    return this.code === "full" /* Full */ || this.code === "partial" /* Partial */;
  }
  get fullySupported() {
    return this.code === "full" /* Full */;
  }
  get compatible() {
    return this.code !== "incompatible" /* Incompatible */;
  }
  _t(to, message) {
    if (to === "partial" /* Partial */) {
      if (this.code === "full" /* Full */) {
        this.code = to;
      }
      if (message) {
        this.fallbacks.push(message);
      }
    } else if (to === "incompatible" /* Incompatible */) {
      if (this.code === "full" /* Full */ || this.code === "partial" /* Partial */) {
        this.code = to;
        if (message && !this.error) {
          this.error = message;
        }
      }
    } else if (to === "invalid" /* Invalid */) {
      this.code = to;
      if (message && !this.error) {
        this.error = message;
      }
  ...
  _fail(schema, validator) {
    if (validator.errors) {
      for (const err of validator.errors) {
        this._t(
          "invalid" /* Invalid */,
          `Failed ${schema.id} validation for ${err.schemaPath}: ${err.message || `Invalid value`}`
        );
      }
    } else {
      this._t("invalid" /* Invalid */, `Failed ${schema.id} validation`);
    }
  }
};
...
var RecordValidator = class {
  ...
  validate(value) {
    ...
    if (!isRecord(value)) {
      res._t("invalid" /* Invalid */, `The passed value is not an object`);
      return res;
    }
    if (!value.$type) {
      res._t(
    ...
    const typeSchema = this.type.find(schemaIdFilter(value.$type));
    if (!typeSchema) {
      res._t(
    ...
    if ("$ext" in value && typeof value.$ext === "object") {
      for (const [extSchemaId, obj] of Object.entries(
        ...
        const extObj = obj;
        const extIsRequired = "$required" in extObj && typeof extObj.$required === "boolean" ? extObj.$required : false;
        let extFallback;
        ...
        if (!extSchema || !extSchema.validate) {
          if (extIsRequired) {
            ...
            res._t("partial" /* Partial */, extFallback);
          }
        } else {
...
var schemaIdFilter = (schemaId) => (s) => s.id === schemaId;
var validator_default = RecordValidator;

// src/record/schemas.ts
var RecordSchemas = class {
  constructor() {
  ...
  add(schemaDef) {
    ...
    const schema = new schema_default(schemaDef);
