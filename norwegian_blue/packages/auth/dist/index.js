      ...
      if (paramsSerializer) {
        ...
        utils2.forEach(params, function serialize(val, key) {
          ...
          utils2.forEach(val, function parseValue(v) {
            ...
            parts.push(encode12(key) + "=" + encode12(v));
          });
        });
    ...
    function genLog(level2, hook) {
      ...
      function LOG(o, ...n) {
        if (typeof o === "object") {
          ...
          this[writeSym](o, format2(msg, formatParams, this[formatOptsSym]), level2);
        } else {
          this[writeSym](null, format2(o === void 0 ? n.shift() : o, n, this[formatOptsSym]), level2);
        }
      }
    }
...
__export(src_exports3, {
  ATP_ABILITIES: () => ATP_ABILITIES,
  ATP_ABILITY_LEVELS: () => ATP_ABILITY_LEVELS,
  AuthStore: () => AuthStore,
  DID_KEY_PLUGINS: () => DID_KEY_PLUGINS,
  EcdsaKeypair: () => EcdsaKeypair,
  Verifier: () => Verifier,
  atpCapability: () => atpCapability,
  atpSemantics: () => atpSemantics,
  encodeUcan: () => encode8,
  isAtpAbility: () => isAtpAbility,
  isAtpCap: () => isAtpCap,
  maintenanceCap: () => maintenanceCap,
  parseAtpAbility: () => parseAtpAbility,
  parseAtpResource: () => parseAtpResource,
  ucans: () => esm_exports,
  vaguerCap: () => vaguerCap,
  verifyAtpUcan: () => verifyAtpUcan,
  verifyFullWritePermission: () => verifyFullWritePermission,
  verifySignature: () => verifySignature,
  verifySignatureUtf8: () => verifySignatureUtf8,
  verifyUcan: () => verifyUcan,
  writeCap: () => writeCap
});
...
var from = ({ name: name2, prefix, encode: encode12, decode: decode8 }) => new Codec(name2, prefix, encode12, decode8);
var baseX = ({ prefix, name: name2, alphabet: alphabet2 }) => {
  const { encode: encode12, decode: decode8 } = base_x_default(alphabet2, name2);
  return from({
    prefix,
    name: name2,
    encode: encode12,
...
function createCodec(name2, prefix, encode12, decode8) {
  return {
    name: name2,
    prefix,
    encoder: {
...
function prf2(selector, ability) {
  return {
    with: prf(selector),
    can: ability
  };
...
function encode7(cap) {
  return {
    with: encode6(cap.with),
    can: encode5(cap.can)
  };
}
function parse4(cap) {
...
function encodePayload(payload) {
  const payloadWithEncodedCaps = Object.assign(Object.assign({}, payload), { att: payload.att.map(encode7) });
...
var validateProofs = (plugins2) => function(ucan, opts) {
  return __asyncGenerator(this, arguments, function* () {
    ...
    for (const prf3 of ucan.payload.prf) {
      try {
        const proof = yield __await(validate(plugins2)(prf3, opts));
...
var delegationChains = (plugins2) => function(semantics, ucan, isRevoked = () => __awaiter3(this, void 0, void 0, function* () {
  ...
  return __asyncGenerator2(this, arguments, function* () {
    if (yield __await2(isRevoked(ucan))) {
      yield yield __await2(new Error(`UCAN Revoked: ${encode8(ucan)}`));
  ...
  return class Builder {
    ...
    delegateCapability(requiredCapability, storeOrProof, semantics) {
      ...
      if (isProof(storeOrProof)) {
        ...
        return new Builder(this.state, Object.assign(Object.assign({}, this.defaultable), { capabilities: [...this.defaultable.capabilities, requiredCapability], proofs: this.defaultable.proofs.find((p) => encode8(p) === encode8(ucan)) == null ? [...this.defaultable.proofs, ucan] : this.defaultable.proofs }));
      } else {
        ...
        if (result != null) {
          ...
          const ucanEncoded = encode8(ucan);
          return new Builder(this.state, Object.assign(Object.assign({}, this.defaultable), { capabilities: [...this.defaultable.capabilities, requiredCapability], proofs: this.defaultable.proofs.find((proof) => encode8(proof) === ucanEncoded) == null ? [...this.defaultable.proofs, ucan] : this.defaultable.proofs }));
        } else {
    ...
    buildPayload() {
      ...
      return buildPayload({
        ...
        proofs: this.defaultable.proofs.map((proof) => encode8(proof))
      });
    }
    build() {
  ...
  return class Store {
    ...
    add(ucan) {
      ...
      return __awaiter6(this, void 0, void 0, function* () {
        ...
        const encoded = encode8(ucan);
        if (byAudience.find((stored) => encode8(stored.processedUcan) === encoded) != null) {
          return;
        }
        const chains = [];
        try {
...
var parsePrefixedBytes = (did) => {
  ...
  return fromString2(did.slice(BASE58_DID_PREFIX.length), "base58btc");
};
...
var verifyUcan = (ucanApi) => async (token, opts) => {
  const encoded = typeof token === "string" ? token : encode8(token);
...
var AuthStore = class {
  ...
  async createUcanForCaps(audience, caps, lifetime = MONTH_IN_SEC) {
    ...
    for (const prf3 of proofs) {
      builder = builder.delegateCapability(prf3.cap, prf3.prf, atpSemantics);
    }
    return builder.build();
  }
  async vaguestProofForCap(cap) {
    const prf3 = await this.findProof(cap);
    if (prf3 === null)
      return null;
