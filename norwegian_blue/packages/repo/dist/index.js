...
var require_err = __commonJS({
  "../../node_modules/pino-std-serializers/lib/err.js"(exports, module2) {
    ...
    function errSerializer(err2) {
      ...
      if (global.AggregateError !== void 0 && err2 instanceof global.AggregateError && Array.isArray(err2.errors)) {
...
var require_fast_redact = __commonJS({
  "../../node_modules/fast-redact/index.js"(exports, module2) {
    ...
    function fastRedact(opts = {}) {
      ...
      const { wildcards, wcLen, secret } = parse7({ paths, censor });
  ...
  "../../node_modules/sonic-boom/index.js"(exports, module2) {
    ...
    function openFile(file, sonic) {
      ...
      if (sonic.sync) {
        ...
        fs3.mkdir(path.dirname(file), { recursive: true }, (err2) => {
          if (err2)
    ...
    function SonicBoom(opts) {
      ...
      this.release = (err2, n) => {
        if (err2) {
          if (err2.code === "EAGAIN" && this.retryEAGAIN(err2, this._writingBuf.length, this._len - this._writingBuf.length)) {
            if (this.sync) {
              try {
    ...
    SonicBoom.prototype.flushSync = function() {
      ...
      while (this._bufs.length) {
        ...
        try {
          ...
          if (err2.code !== "EAGAIN" || !this.retryEAGAIN(err2, buf2.length, this._len - buf2.length)) {
            throw err2;
          }
          sleep(BUSY_WRITE_TIMEOUT);
        }
  ...
  "../../node_modules/thread-stream/index.js"(exports, module2) {
    ...
    var ThreadStream = class extends EventEmitter {
      ...
      flush(cb) {
        ...
        wait2(this[kImpl].state, READ_INDEX, writeIndex, Infinity, (err2, res) => {
          if (err2) {
            destroy(this, err2);
    ...
    function write(stream, data, cb) {
      ...
      Atomics.store(stream[kImpl].state, WRITE_INDEX, current + length3);
...
var require_tools = __commonJS({
  "../../node_modules/pino/lib/tools.js"(exports, module2) {
    ...
    function genLog(level3, hook) {
      if (!hook)
        return LOG;
      return function hookWrappedLog(...args) {
        hook.call(this, args, LOG, level3);
      };
      function LOG(o, ...n) {
        if (typeof o === "object") {
          let msg = o;
          ...
          this[writeSym](o, format2(msg, formatParams, this[formatOptsSym]), level3);
        } else {
          this[writeSym](null, format2(o === void 0 ? n.shift() : o, n, this[formatOptsSym]), level3);
        }
      }
    }
    ...
    function buildFormatters(level3, bindings, log) {
      return {
        level: level3,
        bindings,
        log
  ...
  "../../node_modules/pino/lib/levels.js"(exports, module2) {
    ...
    function genLsCache(instance) {
      ...
      for (const label in labels) {
        const level3 = formatter(labels[label], Number(label));
        cache[label] = JSON.stringify(level3).slice(0, -1);
      }
      instance[lsCacheSym] = cache;
      return instance;
    }
    function isStandardLevel(level3, useOnlyCustomLevels) {
    ...
    function setLevel(level3) {
      const { labels, values } = this.levels;
      if (typeof level3 === "number") {
        if (labels[level3] === void 0)
          throw Error("unknown level value" + level3);
        level3 = labels[level3];
      }
      if (values[level3] === void 0)
        throw Error("unknown level " + level3);
      const preLevelVal = this[levelValSym];
      const levelVal = this[levelValSym] = values[level3];
    ...
    function assertDefaultLevelFound(defaultLevel, customLevels, useOnlyCustomLevels) {
      if (typeof defaultLevel === "number") {
        const values = [].concat(
          ...
          useOnlyCustomLevels ? [] : Object.keys(nums).map((level3) => +level3),
          Infinity
        );
  ...
  "../../node_modules/pino/lib/proto.js"(exports, module2) {
    ...
    function child(bindings2, options) {
      ...
      if (options.hasOwnProperty("formatters")) {
        const { level: level3, bindings: chindings, log } = options.formatters;
        instance[formattersSym] = buildFormatters(
          level3 || formatters.level,
...
var require_multistream = __commonJS({
  "../../node_modules/pino/lib/multistream.js"(exports, module2) {
    ...
    function multistream(streamsArray, opts) {
      ...
      function write(data) {
        let dest;
        const level3 = this.lastLevel;
        const { streams } = this;
        let recordedLevel = 0;
        let stream;
        for (let i = initLoopVar(streams.length, opts.dedupe); checkLoopVar(i, streams.length, opts.dedupe); i = adjustLoopVar(i, opts.dedupe)) {
          dest = streams[i];
          if (dest.level <= level3) {
      ...
      function add(dest) {
        ...
        if (typeof dest.levelVal === "number") {
          level3 = dest.levelVal;
        } else if (typeof dest.level === "string") {
          level3 = levels2[dest.level];
        } else if (typeof dest.level === "number") {
          level3 = dest.level;
        } else {
          level3 = DEFAULT_INFO_LEVEL;
        }
        const dest_ = {
          stream: stream_,
          level: level3,
    ...
    function initLoopVar(length3, dedupe) {
      return dedupe ? length3 - 1 : 0;
    }
    function adjustLoopVar(i, dedupe) {
      return dedupe ? i - 1 : i + 1;
    }
    function checkLoopVar(i, length3, dedupe) {
      return dedupe ? i >= 0 : i < length3;
    }
...
var require_pino = __commonJS({
  "../../node_modules/pino/pino.js"(exports, module2) {
    ...
    function pino2(...args) {
      ...
      assertDefaultLevelFound(level3, customLevels, useOnlyCustomLevels);
...
var require_BigInteger = __commonJS({
  "../../node_modules/big-integer/BigInteger.js"(exports, module2) {
    var bigInt2 = function(undefined2) {
      ...
      function subtractAny(a, b, sign3) {
        var value;
        if (compareAbs(a, b) >= 0) {
          value = subtract(a, b);
        } else {
      ...
      function subtractSmall(a, b, sign3) {
      ...
      BigInteger.prototype.multiply = function(v) {
        var n = parseValue(v), a = this.value, b = n.value, sign3 = this.sign !== n.sign, abs;
        if (n.isSmall) {
          if (b === 0)
            return Integer[0];
          if (b === 1)
        ...
        if (useKaratsuba(a.length, b.length))
          return new BigInteger(multiplyKaratsuba(a, b), sign3);
        return new BigInteger(multiplyLong(a, b), sign3);
      };
      BigInteger.prototype.times = BigInteger.prototype.multiply;
      function multiplySmallAndArray(a, b, sign3) {
        if (a < BASE) {
          return new BigInteger(multiplySmall(b, a), sign3);
        }
        return new BigInteger(multiplyLong(b, smallToArray(a)), sign3);
      }
      ...
      function divModSmall(value, lambda) {
        var length3 = value.length, quotient = createArray(length3), base3 = BASE, i2, q, remainder, divisor;
        remainder = 0;
      ...
      function parseStringValue(v) {
        ...
        if (!isValid2)
          throw new Error("Invalid integer: " + v);
        if (supportsNativeBigInt) {
          return new NativeBigInt(BigInt(sign3 ? "-" + v : v));
        }
      ...
      if (paramsSerializer) {
        ...
        utils2.forEach(params, function serialize(val, key) {
          ...
          utils2.forEach(val, function parseValue(v) {
            ...
            parts.push(encode14(key) + "=" + encode14(v));
          });
        });
...
var require_enhanceError = __commonJS({
  "../../node_modules/axios/lib/core/enhanceError.js"(exports, module2) {
    "use strict";
    module2.exports = function enhanceError(error2, config2, code3, request, response) {
...
var require_createError = __commonJS({
  "../../node_modules/axios/lib/core/createError.js"(exports, module2) {
    ...
    module2.exports = function createError(message, config2, code3, request, response) {
      var error2 = new Error(message);
      return enhanceError(error2, config2, code3, request, response);
    };
  }
});
...
var require_supports_color = __commonJS({
  "../../node_modules/supports-color/index.js"(exports, module2) {
    ...
    function translateLevel(level3) {
      if (level3 === 0) {
        return false;
      }
      return {
    ...
    function getSupportLevel(stream) {
      const level3 = supportsColor(stream, stream && stream.isTTY);
      return translateLevel(level3);
    }
    module2.exports = {
...
__export(src_exports5, {
  ...
  MstWalker: () => MstWalker,
  PersistentBlockstore: () => PersistentBlockstore,
  Repo: () => Repo,
  RepoStructure: () => RepoStructure,
  cidCreateOp: () => cidCreateOp,
  cidUpdateOp: () => cidUpdateOp,
  cidWriteOp: () => cidWriteOp,
  def: () => def2,
  deleteOp: () => deleteOp,
  getCommitPath: () => getCommitPath,
  mstUtil: () => util_exports3,
  nodeDataDef: () => nodeDataDef,
  recordCreateOp: () => recordCreateOp,
  recordUpdateOp: () => recordUpdateOp,
  recordWriteOp: () => recordWriteOp,
  ucanForOperation: () => ucanForOperation,
...
var flattenUint8Arrays = (arrs) => {
  const length3 = arrs.reduce((acc, cur) => {
    return acc + cur.length;
  }, 0);
  const flattened = new Uint8Array(length3);
  let offset = 0;
  arrs.forEach((arr) => {
...
var from = ({ name: name3, prefix, encode: encode14, decode: decode12 }) => new Codec(name3, prefix, encode14, decode12);
var baseX = ({ prefix, name: name3, alphabet: alphabet2 }) => {
  const { encode: encode14, decode: decode12 } = base_x_default(alphabet2, name3);
  return from({
    prefix,
    name: name3,
    encode: encode14,
...
var CID = class {
  ...
  static asCID(value) {
    if (value instanceof CID) {
      ...
      return new CID(version3, code3, multihash, bytes2 || encodeCID(version3, code3, multihash.bytes));
    } else if (value != null && value[cidSymbol] === true) {
      const { version: version3, multihash, code: code3 } = value;
      const digest2 = decode3(multihash);
      return CID.create(version3, code3, digest2);
    } else {
      return null;
    }
  }
    ...
    switch (version3) {
      case 0: {
        if (code3 !== DAG_PB_CODE) {
          ...
          return new CID(version3, code3, digest2, digest2.bytes);
        }
      }
      case 1: {
        const bytes2 = encodeCID(version3, code3, digest2.bytes);
        return new CID(version3, code3, digest2, bytes2);
      }
      default: {
...
var from2 = ({ name: name3, code: code3, encode: encode14 }) => new Hasher(name3, code3, encode14);
var Hasher = class {
  constructor(name3, code3, encode14) {
    this.name = name3;
    this.code = code3;
...
var decode9 = (multihash) => {
  ...
  const [code3, sizeOffset] = decode8(bytes2);
...
var from3 = ({ name: name3, prefix, encode: encode14, decode: decode12 }) => new Codec2(name3, prefix, encode14, decode12);
var baseX2 = ({ prefix, name: name3, alphabet: alphabet2 }) => {
  const { encode: encode14, decode: decode12 } = base_x_default2(alphabet2, name3);
  return from3({
    prefix,
    name: name3,
    encode: encode14,
...
var decode10 = (string2, alphabet2, bitsPerChar, name3) => {
  const codes = {};
  for (let i = 0; i < alphabet2.length; ++i) {
    codes[alphabet2[i]] = i;
  }
...
var rfc46482 = ({ name: name3, prefix, bitsPerChar, alphabet: alphabet2 }) => {
  return from3({
    ...
    decode(input) {
      return decode10(input, alphabet2, bitsPerChar, name3);
    }
  });
};
...
var CID2 = class {
  ...
  static asCID(value) {
    if (value instanceof CID2) {
      ...
      const { version: version3, code: code3, multihash, bytes: bytes2 } = value;
      return new CID2(version3, code3, multihash, bytes2 || encodeCID2(version3, code3, multihash.bytes));
    } else if (value != null && value[cidSymbol2] === true) {
      const { version: version3, multihash, code: code3 } = value;
      const digest2 = decode9(multihash);
      return CID2.create(version3, code3, digest2);
    } else {
      return null;
    }
  }
    ...
    switch (version3) {
      case 0: {
        if (code3 !== DAG_PB_CODE2) {
          ...
          return new CID2(version3, code3, digest2, digest2.bytes);
        }
      }
      case 1: {
        const bytes2 = encodeCID2(version3, code3, digest2.bytes);
        return new CID2(version3, code3, digest2, bytes2);
      }
      default: {
        throw new Error("Invalid version");
      }
...
var encodeCID2 = (version3, code3, multihash) => {
...
async function readHeader(reader) {
  const length3 = await readVarint(reader);
  if (length3 === 0) {
    throw new Error("Invalid CAR header (zero length)");
  }
  const header = await reader.exactly(length3);
  reader.seek(length3);
...
async function readBlockHead(reader) {
  ...
  let length3 = await readVarint(reader);
  if (length3 === 0) {
    throw new Error("Invalid CAR section (zero length)");
  }
  length3 += reader.pos - start;
  const cid2 = await readCid(reader);
  const blockLength = length3 - (reader.pos - start);
  return {
    cid: cid2,
    length: length3,
    blockLength
...
async function readBlockIndex(reader) {
  ...
  const { cid: cid2, length: length3, blockLength } = await readBlockHead(reader);
  const index = {
    cid: cid2,
    length: length3,
    blockLength,
...
var IpldStore = class {
  ...
  async get(cid2, schema) {
    const value = await this.getUnchecked(cid2);
    try {
      return check_exports.assure(schema, value);
    } catch (err2) {
      throw new Error(
        `Did not find expected object at ${cid2.toString()}: ${err2}`
      );
    }
  }
  async getUnchecked(cid2) {
    const bytes2 = await this.getBytes(cid2);
    return ipldBytesToValue(bytes2);
  }
  async isMissing(cid2) {
    const has = await this.has(cid2);
    return !has;
  }
  async checkMissing(cids) {
    const missing = await util_exports.asyncFilter(cids.toList(), (c) => {
      return this.isMissing(c);
    });
    return new cid_set_default(missing);
  }
  async addToCar(car, cid2) {
    car.put({ cid: cid2, bytes: await this.getBytes(cid2) });
  }
  async loadCar(buf2) {
    const car = await CarReader2.fromBytes(buf2);
  ...
  async loadCarBlocks(car) {
    for await (const block of car.blocks()) {
      await this.putBytes(block.cid, block.bytes);
    }
  }
};
var ipld_store_default = IpldStore;

// src/blockstore/memory-blockstore.ts
var MemoryBlockstore = class extends ipld_store_default {
  constructor() {
    super();
  ...
  async getContents() {
    ...
    for (const key of this.map.keys()) {
      contents[key] = await this.getUnchecked(CID2.parse(key));
    }
    return contents;
  }
};

// src/blockstore/persistent-blockstore.ts
var import_level = __toESM(require("level"));
var PersistentBlockstore = class extends ipld_store_default {
  constructor(location = "blockstore") {
    super();
    this.store = new import_level.default.Level(location, {
      valueEncoding: "view",
      compression: false
    });
  }
  async getBytes(cid2) {
    return this.store.get(cid2.toString(), { valueEncoding: "view" });
  }
  async putBytes(cid2, bytes2) {
    await this.store.put(cid2.toString(), bytes2, { valueEncoding: "view" });
  }
  async has(cid2) {
    try {
      await this.getBytes(cid2);
      return true;
    } catch (_) {
      return false;
    }
...
function createEncoder(writer) {
  return {
    ...
    async writeBlock(block) {
      ...
      await writer.write(new Uint8Array(import_varint4.default.encode(cid2.bytes.length + bytes2.length)));
...
function createCodec(name3, prefix, encode14, decode12) {
  return {
    name: name3,
    prefix,
    encoder: {
...
// ../../node_modules/@ucans/core/dist/esm/util.js
...
// ../../node_modules/@ucans/core/dist/esm/semver.js
function isSemVer(obj) {
  return isRecord(obj) && hasProp(obj, "major") && typeof obj.major === "number" && hasProp(obj, "minor") && typeof obj.minor === "number" && hasProp(obj, "patch") && typeof obj.patch === "number";
}
var NUM_REGEX = /^0|[1-9]\d*/;
var matchesRegex = (regex) => (str) => {
  const m = str.match(regex);
  if (!m)
    return false;
  return m[0].length === str.length;
};
function parse(version3) {
...
function lt(left, right) {
  return compare3(left, right) === LT;
}
function gte(left, right) {
  return !lt(left, right);
}

// ../../node_modules/@ucans/core/dist/esm/capability/ability.js
var ability_exports = {};
...
// ../../node_modules/@ucans/core/dist/esm/capability/super-user.js
var SUPERUSER = "*";

// ../../node_modules/@ucans/core/dist/esm/capability/ability.js
var SEPARATOR = "/";
var REDELEGATE = { namespace: "ucan", segments: ["DELEGATE"] };
function isAbility(obj) {
  return obj === SUPERUSER || isRecord(obj) && hasProp(obj, "namespace") && typeof obj.namespace === "string" && hasProp(obj, "segments") && Array.isArray(obj.segments) && obj.segments.every((s) => typeof s === "string");
}
...
function encode10(ability) {
  switch (ability) {
    case SUPERUSER:
      return ability;
    default:
      return joinSegments([ability.namespace, ...ability.segments]);
  }
}
function parse2(ability) {
  switch (ability) {
    case SUPERUSER:
      return SUPERUSER;
    default: {
...
// ../../node_modules/@ucans/core/dist/esm/capability/resource-pointer.js
var SEPARATOR2 = ":";
...
function encode11(pointer) {
  return `${pointer.scheme}${SEPARATOR2}${pointer.hierPart}`;
}
function parse3(pointer) {
  const [scheme, ...hierPart] = pointer.split(SEPARATOR2);
  return { scheme, hierPart: hierPart.join(SEPARATOR2) };
}

// ../../node_modules/@ucans/core/dist/esm/capability/index.js
function isCapability(obj) {
  return isRecord(obj) && hasProp(obj, "with") && isResourcePointer(obj.with) && hasProp(obj, "can") && isAbility(obj.can);
}
...
function my2(resource) {
  return {
    with: my(resource),
    can: SUPERUSER
  };
}
function encode12(cap) {
  return {
    with: encode11(cap.with),
    can: encode10(cap.can)
  };
}
function parse4(cap) {
  return {
    with: parse3(cap.with),
    can: parse2(cap.can)
  };
}

// ../../node_modules/@ucans/core/dist/esm/types.js
...
function isUcanHeader(obj) {
  return isRecord(obj) && hasProp(obj, "alg") && typeof obj.alg === "string" && hasProp(obj, "typ") && typeof obj.typ === "string" && hasProp(obj, "ucv") && isSemVer(obj.ucv);
}
function isUcanPayload(obj) {
  return isRecord(obj) && hasProp(obj, "iss") && typeof obj.iss === "string" && hasProp(obj, "aud") && typeof obj.aud === "string" && hasProp(obj, "exp") && typeof obj.exp === "number" && (!hasProp(obj, "nbf") || typeof obj.nbf === "number") && (!hasProp(obj, "nnc") || typeof obj.nnc === "string") && hasProp(obj, "att") && Array.isArray(obj.att) && obj.att.every((a) => isCapability(a) || isEncodedCapability(a)) && (!hasProp(obj, "fct") || Array.isArray(obj.fct) && obj.fct.every(isRecord)) && hasProp(obj, "prf") && Array.isArray(obj.prf) && obj.prf.every((str) => typeof str === "string");
}

// ../../node_modules/@ucans/core/dist/esm/compatibility.js
var VERSION_0_3 = { major: 0, minor: 3, patch: 0 };
function isUcanHeader_0_3_0(obj) {
  return isRecord(obj) && hasProp(obj, "alg") && typeof obj.alg === "string" && hasProp(obj, "typ") && typeof obj.typ === "string" && hasProp(obj, "uav") && typeof obj.uav === "string";
}
function isUcanPayload_0_3_0(obj) {
  return isRecord(obj) && hasProp(obj, "iss") && typeof obj.iss === "string" && hasProp(obj, "aud") && typeof obj.aud === "string" && (!hasProp(obj, "nbf") || typeof obj.nbf === "number") && hasProp(obj, "exp") && typeof obj.exp === "number" && hasProp(obj, "rsc") && (typeof obj.rsc === "string" || isRecord(obj)) && hasProp(obj, "ptc") && typeof obj.ptc === "string" && (!hasProp(obj, "prf") || typeof obj.prf === "string");
}
function handleCompatibility(header, payload) {
  ...
  let version3 = "0.8.1";
  if (!hasProp(header, "ucv") || typeof header.ucv !== "string") {
    if (!hasProp(header, "uav") || typeof header.uav !== "string") {
      ...
      att: (() => {
        ...
        return Object.keys(resources).map((rscKey) => {
          return {
            ...
            can: payload.ptc === SUPERUSER ? SUPERUSER : { namespace: rscKey, segments: [payload.ptc] }
          };
        });
      })(),
...
// ../../node_modules/@ucans/core/dist/esm/token.js
...
var TYPE = "JWT";
var VERSION = { major: 0, minor: 8, patch: 1 };
var build = (plugins2) => (params) => {
  const keypair = params.issuer;
  const payload = buildPayload(Object.assign(Object.assign({}, params), { issuer: keypair.did() }));
  return signWithKeypair(plugins2)(payload, keypair);
};
function buildPayload(params) {
  ...
  return {
    ...
    iss: issuer,
    nbf: notBefore,
    nnc: addNonce ? generateNonce() : void 0,
    prf: proofs
  };
}
var sign = (plugins2) => (payload, jwtAlg, signFn) => __awaiter2(void 0, void 0, void 0, function* () {
  const header = {
    alg: jwtAlg,
    typ: TYPE,
    ucv: VERSION
  };
  if (!plugins2.verifyIssuerAlg(payload.iss, jwtAlg)) {
    throw new Error("The issuer's key type must match the given key type.");
  }
  const encodedHeader = encodeHeader(header);
  const encodedPayload = encodePayload(payload);
  const signedData = `${encodedHeader}.${encodedPayload}`;
  const toSign = fromString3(signedData, "utf8");
  const sig = yield signFn(toSign);
  return Object.freeze({
    header,
    payload,
    signedData,
    signature: toString3(sig, "base64url")
  });
});
var signWithKeypair = (plugins2) => (payload, keypair) => {
  return sign(plugins2)(payload, keypair.jwtAlg, (data) => keypair.sign(data));
};
function encode13(ucan) {
  return `${ucan.signedData}.${ucan.signature}`;
}
function encodeHeader(header) {
  const headerFormatted = Object.assign(Object.assign({}, header), { ucv: format(header.ucv) });
  return toString3(fromString3(JSON.stringify(headerFormatted), "utf8"), "base64url");
}
function encodePayload(payload) {
  const payloadWithEncodedCaps = Object.assign(Object.assign({}, payload), { att: payload.att.map(encode12) });
  return toString3(fromString3(JSON.stringify(payloadWithEncodedCaps), "utf8"), "base64url");
}
function parse5(encodedUcan) {
  ...
  let headerObject;
  try {
    headerJson = toString3(fromString3(encodedHeader, "base64url"), "utf8");
  } catch (_a) {
  ...
  let payloadObject;
  try {
    payloadJson = toString3(fromString3(encodedPayload, "base64url"), "utf8");
  } catch (_c) {
  ...
  const parsedAttenuations = payload.att.reduce((acc, cap) => {
    return isEncodedCapability(cap) ? [...acc, parse4(cap)] : isCapability(cap) ? [...acc, cap] : acc;
  }, []);
  return {
    header,
...
var validate = (plugins2) => (encodedUcan, opts) => __awaiter2(void 0, void 0, void 0, function* () {
  const { checkIssuer = true, checkSignature = true, checkIsExpired = true, checkIsTooEarly = true } = opts !== null && opts !== void 0 ? opts : {};
  const { header, payload } = parse5(encodedUcan);
  const [encodedHeader, encodedPayload, signature] = encodedUcan.split(".");
  if (checkIssuer) {
    const validIssuer = plugins2.verifyIssuerAlg(payload.iss, header.alg);
    if (!validIssuer) {
      throw new Error(`Invalid UCAN: ${encodedUcan}: Issuer key type does not match UCAN's \`alg\` property.`);
    }
  }
  if (checkSignature) {
    const sigBytes = fromString3(signature, "base64url");
    const data = fromString3(`${encodedHeader}.${encodedPayload}`, "utf8");
    const validSig = yield plugins2.verifySignature(payload.iss, data, sigBytes);
    if (!validSig) {
...
var validateProofs = (plugins2) => function(ucan, opts) {
  return __asyncGenerator(this, arguments, function* () {
    const { checkAddressing = true, checkTimeBoundsSubset = true, checkVersionMonotonic = true } = opts || {};
    for (const prf2 of ucan.payload.prf) {
      try {
        const proof = yield __await(validate(plugins2)(prf2, opts));
        ...
        if (checkVersionMonotonic && lt(ucan.header.ucv, proof.header.ucv)) {
...
// ../../node_modules/@ucans/core/dist/esm/attenuation.js
...
var delegationChains = (plugins2) => function(semantics, ucan, isRevoked = () => __awaiter3(this, void 0, void 0, function* () {
  return false;
})) {
  return __asyncGenerator2(this, arguments, function* () {
    if (yield __await2(isRevoked(ucan))) {
      yield yield __await2(new Error(`UCAN Revoked: ${encode13(ucan)}`));
      return yield __await2(void 0);
    }
    yield __await2(yield* __asyncDelegator(__asyncValues2(capabilitiesFromParenthood(ucan))));
    yield __await2(yield* __asyncDelegator(__asyncValues2(capabilitiesFromDelegation(plugins2, semantics, ucan, isRevoked))));
  });
};
...
function* capabilitiesFromParenthood(ucan) {
  for (const capability of ucan.payload.att) {
    switch (capability.with.scheme.toLowerCase()) {
      case "my": {
        const scope = capability.with.hierPart === SUPERUSER ? SUPERUSER : { scheme: capability.with.hierPart, ability: capability.can };
        yield {
...
function capabilitiesFromDelegation(plugins2, semantics, ucan, isRevoked) {
  return __asyncGenerator2(this, arguments, function* capabilitiesFromDelegation_1() {
    var e_1, _a;
    let proofIndex = 0;
    try {
      for (var _b = __asyncValues2(validateProofs(plugins2)(ucan)), _c; _c = yield __await2(_b.next()), !_c.done; ) {
        const proof = _c.value;
        if (proof instanceof Error) {
          yield yield __await2(proof);
          continue;
        }
        for (const capability of ucan.payload.att) {
          try {
            switch (capability.with.scheme.toLowerCase()) {
              case "my":
                continue;
              case "as": {
                yield __await2(yield* __asyncDelegator(__asyncValues2(handleAsDelegation(plugins2, semantics, capability, ucan, proof, isRevoked))));
                break;
              }
              case "prf": {
                yield __await2(yield* __asyncDelegator(__asyncValues2(handlePrfDelegation(plugins2, semantics, capability, ucan, proof, proofIndex, isRevoked))));
                break;
              }
              default: {
                yield __await2(yield* __asyncDelegator(__asyncValues2(handleNormalDelegation(plugins2, semantics, capability, ucan, proof, isRevoked))));
              }
            }
          } catch (e) {
            yield yield __await2(error2(e));
          }
        }
        proofIndex++;
      }
...
function handleAsDelegation(plugins2, semantics, capability, ucan, proof, isRevoked) {
  return __asyncGenerator2(this, arguments, function* handleAsDelegation_1() {
    var e_2, _a;
    const split = capability.with.hierPart.split(":");
    const scheme = split[split.length - 1];
    const ownershipDID = split.slice(0, -1).join(":");
    const scope = scheme === SUPERUSER ? SUPERUSER : { scheme, ability: capability.can };
    try {
      for (var _b = __asyncValues2(delegationChains(plugins2)(semantics, proof, isRevoked)), _c; _c = yield __await2(_b.next()), !_c.done; ) {
        const delegationChain = _c.value;
        if (delegationChain instanceof Error) {
          yield yield __await2(delegationChain);
          continue;
        }
...
function handlePrfDelegation(plugins2, semantics, capability, ucan, proof, proofIndex, isRevoked) {
  return __asyncGenerator2(this, arguments, function* handlePrfDelegation_1() {
    var e_3, _a;
    if (capability.with.hierPart !== SUPERUSER && parseInt(capability.with.hierPart, 10) !== proofIndex) {
      return yield __await2(void 0);
    }
    try {
      for (var _b = __asyncValues2(delegationChains(plugins2)(semantics, proof, isRevoked)), _c; _c = yield __await2(_b.next()), !_c.done; ) {
...
function handleNormalDelegation(plugins2, semantics, capability, ucan, proof, isRevoked) {
  return __asyncGenerator2(this, arguments, function* handleNormalDelegation_1() {
    var e_4, _a;
    try {
      for (var _b = __asyncValues2(delegationChains(plugins2)(semantics, proof, isRevoked)), _c; _c = yield __await2(_b.next()), !_c.done; ) {
        const delegationChain = _c.value;
...
// ../../node_modules/@ucans/core/dist/esm/verify.js
...
var ok = (k) => ({ ok: true, value: k });
var err = (e) => ({ ok: false, error: e });
var verify = (plugins2) => (ucan, options) => __awaiter4(void 0, void 0, void 0, function* () {
  var e_1, _a;
  var _b, _c, _d;
  const { audience, requiredCapabilities } = options;
  const semantics = (_b = options.semantics) !== null && _b !== void 0 ? _b : equalCanDelegate;
  const isRevoked = (_c = options.isRevoked) !== null && _c !== void 0 ? _c : () => __awaiter4(void 0, void 0, void 0, function* () {
    return false;
  });
  ...
  try {
    const decoded = yield validate(plugins2)(ucan);
    ...
    try {
      loop:
        for (var _e = __asyncValues3(delegationChains(plugins2)(semantics, decoded, isRevoked)), _f; _f = yield _e.next(), !_f.done; ) {
          ...
          for (const expected of remaining) {
            if (capabilityCanBeDelegated(semantics, expected.capability, delegationChain) && rootIssuer(delegationChain) === expected.rootIssuer) {
...
// ../../node_modules/@ucans/core/dist/esm/builder.js
...
function isBuildableState(obj) {
  return isRecord(obj) && hasProp(obj, "issuer") && isKeypair(obj.issuer) && hasProp(obj, "audience") && typeof obj.audience === "string" && hasProp(obj, "expiration") && typeof obj.expiration === "number";
}
function isCapabilityLookupCapableState(obj) {
  return isRecord(obj) && hasProp(obj, "issuer") && isKeypair(obj.issuer) && hasProp(obj, "expiration") && typeof obj.expiration === "number";
}
var mkBuilderClass = (plugins2) => {
  return class Builder {
    ...
    withLifetimeInSeconds(seconds) {
      ...
      return this.withExpiration(Math.floor(Date.now() / 1e3) + seconds);
    }
    ...
    withNotBefore(notBeforeTimestamp) {
      ...
      if (hasProp(this.state, "expiration") && typeof this.state.expiration === "number" && this.state.expiration < notBeforeTimestamp) {
    ...
    withFact(fact, ...facts) {
      if (!isRecord(fact) || facts.some((fct) => !isRecord(fct))) {
    ...
    delegateCapability(requiredCapability, storeOrProof, semantics) {
      ...
      function isProof(proof) {
        return hasProp(proof, "capability") || hasProp(proof, "ownershipDID");
      }
      if (isProof(storeOrProof)) {
        ...
        return new Builder(this.state, Object.assign(Object.assign({}, this.defaultable), { capabilities: [...this.defaultable.capabilities, requiredCapability], proofs: this.defaultable.proofs.find((p) => encode13(p) === encode13(ucan)) == null ? [...this.defaultable.proofs, ucan] : this.defaultable.proofs }));
      } else {
        ...
        if (result != null) {
          ...
          const ucanEncoded = encode13(ucan);
          return new Builder(this.state, Object.assign(Object.assign({}, this.defaultable), { capabilities: [...this.defaultable.capabilities, requiredCapability], proofs: this.defaultable.proofs.find((proof) => encode13(proof) === ucanEncoded) == null ? [...this.defaultable.proofs, ucan] : this.defaultable.proofs }));
        } else {
    ...
    buildPayload() {
      ...
      return buildPayload({
        ...
        proofs: this.defaultable.proofs.map((proof) => encode13(proof))
      });
    }
    build() {
      return __awaiter5(this, void 0, void 0, function* () {
        ...
        return yield signWithKeypair(plugins2)(payload, this.state.issuer);
      });
    }
  };
};
...
// ../../node_modules/@ucans/core/dist/esm/store.js
  ...
  return class Store {
    ...
    static fromTokens(knownSemantics, tokens) {
      ...
      return __awaiter6(this, void 0, void 0, function* () {
        ...
        try {
          for (tokens_1 = __asyncValues4(tokens); tokens_1_1 = yield tokens_1.next(), !tokens_1_1.done; ) {
            const encodedUcan = tokens_1_1.value;
            const ucan = yield validate(plugins2)(encodedUcan);
    ...
    add(ucan) {
      ...
      return __awaiter6(this, void 0, void 0, function* () {
        ...
        const encoded = encode13(ucan);
        if (byAudience.find((stored) => encode13(stored.processedUcan) === encoded) != null) {
          return;
        }
        const chains = [];
        try {
          for (var _c = __asyncValues4(delegationChains(plugins2)(this.knownSemantics, ucan)), _d; _d = yield _c.next(), !_d.done; ) {
    ...
    *findWithCapability(audience, requiredCapability, requiredIssuer) {
      ...
      for (const cacheElement of cache) {
        for (const delegationChain of cacheElement.capabilities) {
          if (capabilityCanBeDelegated(this.knownSemantics, requiredCapability, delegationChain) && rootIssuer(delegationChain) === requiredIssuer) {
...
// ../../node_modules/@ucans/core/dist/esm/plugins.js
...
var Plugins = class {
  ...
  verifyIssuerAlg(did, jwtAlg) {
    ...
    if (didMethod === "key") {
      const bytes2 = parsePrefixedBytes(did);
      for (const keyPlugin of this.keys) {
        if (hasPrefix(bytes2, keyPlugin.prefix)) {
...
var hasPrefix = (prefixedKey, prefix) => {
  return equals5(prefix, prefixedKey.subarray(0, prefix.byteLength));
};
...
var parsePrefixedBytes = (did) => {
  ...
  return fromString3(did.slice(BASE58_DID_PREFIX.length), "base58btc");
};
...
// ../../node_modules/@ucans/core/dist/esm/index.js
var getPluginInjectedApi = (plugins2) => {
  const build2 = build(plugins2);
  const sign3 = sign(plugins2);
  const signWithKeypair2 = signWithKeypair(plugins2);
  const validate2 = validate(plugins2);
  const validateProofs2 = validateProofs(plugins2);
  const verify4 = verify(plugins2);
  const delegationChains2 = delegationChains(plugins2);
  const Builder = builder_default(plugins2);
  const Store = store_default(plugins2);
  return {
    build: build2,
    sign: sign3,
    signWithKeypair: signWithKeypair2,
    validate: validate2,
    validateProofs: validateProofs2,
    verify: verify4,
    delegationChains: delegationChains2,
    Builder,
    Store
  };
};
...
var verifyDidSig = async (did, data, sig) => {
  ...
  return verify3(keyBytes, data, sig);
};
var verify3 = async (publicKey, data, sig) => {
...
var verifyDidSig2 = async (did, data, sig) => {
  ...
  return verify2(sig, msgHash, keyBytes);
};
...
var parseDidKey = (did) => {
  ...
  const plugin = plugins_default.find((p) => hasPrefix2(prefixedBytes, p.prefix));
  if (!plugin) {
...
var formatDidKey = (jwtAlg, keyBytes) => {
  ...
  const prefixedBytes = concat2([plugin.prefix, keyBytes]);
  return DID_KEY_BASE58_PREFIX + toString3(prefixedBytes, "base58btc");
};
var hasPrefix2 = (bytes2, prefix) => {
  return equals5(prefix, bytes2.subarray(0, prefix.byteLength));
};
...
var multibaseToBytes = (mb) => {
  ...
  switch (base3) {
    case "f":
      return fromString3(key, "base16");
    case "F":
      return fromString3(key, "base16upper");
    case "b":
      return fromString3(key, "base32");
    case "B":
      return fromString3(key, "base32upper");
    case "z":
      return fromString3(key, "base58btc");
    case "m":
      return fromString3(key, "base64");
    case "u":
      return fromString3(key, "base64url");
    case "U":
      return fromString3(key, "base64urlpad");
    default:
...
var EcdsaKeypair = class {
  ...
  publicKeyStr(encoding = "base64pad") {
    return toString3(this.publicKey, encoding);
  }
  did() {
    return formatDidKey(this.jwtAlg, this.publicKey);
  }
  async sign(msg) {
    const buf2 = await webcrypto.subtle.sign(
      { name: "ECDSA", hash: { name: "SHA-256" } },
...
// ../auth/src/signatures.ts
var verifySignature = (plugins2) => async (did, data, sig) => {
  return plugins2.verifySignature(did, data, sig);
};
var verifySignatureUtf8 = (plugins2) => async (did, data, sig) => {
  const dataBytes = fromString3(data, "utf8");
  const sigBytes = fromString3(sig, "base64url");
  return verifySignature(plugins2)(did, dataBytes, sigBytes);
};

// ../auth/src/atp-semantics.ts
var ATP_ABILITY_LEVELS = {
  SUPER_USER: 2,
  WRITE: 1,
  MAINTENANCE: 0
};
var ATP_ABILITIES = Object.keys(ATP_ABILITY_LEVELS);
var isAtpAbility = (ability) => {
  if (!ability_exports.isAbility(ability))
    return false;
  if (ability === ability_exports.SUPERUSER)
    return true;
  const abilitySegment = ability.segments[0];
  const isAtpAbilitySegment = !!abilitySegment && ATP_ABILITIES.includes(abilitySegment);
  return isAtpAbilitySegment && ability.namespace.toLowerCase() === "atp";
};
var parseAtpAbility = (ability) => {
  if (ability === ability_exports.SUPERUSER)
    return "SUPER_USER";
  if (isAtpAbility(ability))
    return ability.segments[0];
  return null;
};
var atpCapability = (resource, ability) => {
  return {
    with: { scheme: "at", hierPart: resource },
    can: { namespace: "atp", segments: [ability] }
  };
};
var parseAtpResource = (pointer) => {
  if (pointer.scheme !== "at")
    return null;
...
// ../auth/src/atp-capabilities.ts
var writeCap = (did, collection, record) => {
  let resource = did;
  if (collection) {
    resource += "/" + collection;
  }
...
var vaguerCap = (cap) => {
  ...
  if (rsc.collection === "*")
    return null;
  if (rsc.record === "*")
    return writeCap(rsc.did);
  return writeCap(rsc.did, rsc.collection);
};

// ../auth/src/verify.ts
var verifyUcan = (ucanApi) => async (token, opts) => {
  const encoded = typeof token === "string" ? token : encode13(token);
...
var verifyAtpUcan = (ucanApi) => async (token, audience, cap) => {
...
var verifyFullWritePermission = (ucanApi) => async (token, audience, repoDid) => {
  return verifyAtpUcan(ucanApi)(token, audience, writeCap(repoDid));
};

// ../auth/src/consts.ts
var MIN_IN_SEC = 60;
var HOUR_IN_SEC = MIN_IN_SEC * 60;
var DAY_IN_SEC = HOUR_IN_SEC * 24;
var MONTH_IN_SEC = DAY_IN_SEC * 30;
var YEAR_IN_SEC = DAY_IN_SEC * 365;

// ../auth/src/auth-store.ts
var AuthStore = class {
  constructor(ucanApi, keypair, tokens, controlledDid) {
    this.ucanStore = null;
  ...
  async canSignForDid(did) {
    if (did === this.controlledDid)
      return true;
    if (did === await this.keypairDid())
      return true;
    return false;
  }
  async sign(data) {
  ...
  async createUcan(audience, cap, lifetime = MONTH_IN_SEC) {
    const keypair = await this.getKeypair();
    const ucanStore = await this.getUcanStore();
    return this.ucanApi.Builder.create().issuedBy(keypair).toAudience(audience).withLifetimeInSeconds(lifetime).delegateCapability(cap, ucanStore).build();
  }
  async createUcanForCaps(audience, caps, lifetime = MONTH_IN_SEC) {
    const proofs = [];
    ...
    const keypair = await this.getKeypair();
    let builder = this.ucanApi.Builder.create().issuedBy(keypair).toAudience(audience).withLifetimeInSeconds(lifetime);
    for (const prf2 of proofs) {
      builder = builder.delegateCapability(prf2.cap, prf2.prf, atpSemantics);
    }
    return builder.build();
  }
  async vaguestProofForCap(cap) {
    const prf2 = await this.findProof(cap);
    if (prf2 === null)
      return null;
  ...
  async claimFull() {
    ...
    const token = await this.ucanApi.Builder.create().issuedBy(keypair).toAudience(ownDid).withLifetimeInSeconds(YEAR_IN_SEC).claimCapability(writeCap(ownDid)).build();
    await this.addUcan(token);
    return token;
  }
};
var auth_store_default = AuthStore;

// ../did-resolver/src/web/resolver.ts
var import_axios = __toESM(require_axios2());

// ../did-resolver/src/errors.ts
var error = (errorType) => {
  return {
...
// ../did-resolver/src/plc/resolver.ts
...
// ../did-resolver/src/logger.ts
var logger = subsystemLogger("did-resolver");
var logger_default = logger;

// ../did-resolver/src/resolver.ts
var DidResolver = class {
  ...
  async ensureResolveDid(did, options = {}) {
    ...
    if (result.didResolutionMetadata.error || result.didDocument === null) {
      const err2 = result.didResolutionMetadata.error || "notFound";
      logger_default.info({ err: err2, did }, "could not resolve did");
      throw new Error(`Could not resolve DID (${did}): ${err2}`);
    }
...
// ../auth/src/verifier.ts
var DID_KEY_PLUGINS = [p256Plugin];
var Verifier = class {
  constructor(opts = {}) {
    const {
      additionalDidKeys = [],
    ...
    const resolver2 = opts.didResolver ?? new DidResolver({
      plcUrl,
    ...
    const methodPlugins = {
      ...
      verifySignature: async (did, data, sig) => {
        const atpData = await resolver2.resolveAtpData(did);
        return this.verifySignature(atpData.signingKey, data, sig);
      }
    };
    const plugins2 = new Plugins(
    ...
    this.ucanApi = getPluginInjectedApi(plugins2);
    this.plugins = plugins2;
  }
  loadAuthStore(keypair, tokens, controlledDid) {
    return new auth_store_default(this.ucanApi, keypair, tokens, controlledDid);
  }
  async createTempAuthStore(tokens = []) {
...
// src/logger.ts
var logger2 = subsystemLogger("repo");
var logger_default2 = logger2;

// src/collection.ts
var Collection = class {
  constructor(repo, nsid) {
    this.repo = repo;
  ...
  async getRecord(key) {
    const cid2 = await this.repo.data.get(this.dataIdForRecord(key));
    if (cid2 === null)
      return null;
    return this.repo.blockstore.getUnchecked(cid2);
  }
  async listRecords(count, after, before) {
  ...
  async createRecord(record, rkey) {
    const recordKey = rkey || TID.nextStr();
    const cid2 = await this.repo.blockstore.put(record);
    await this.repo.safeCommit({
      action: "create",
      collection: this.nsid.toString(),
      rkey: recordKey,
      cid: cid2
    });
    ...
    return { rkey: recordKey, cid: cid2 };
  }
  async updateRecord(rkey, record) {
    const cid2 = await this.repo.blockstore.put(record);
    await this.repo.safeCommit({
      action: "update",
      collection: this.nsid.toString(),
...
// src/types.ts
var repoMeta = mod.object({
  did: mod.string(),
  version: mod.number(),
  datastore: mod.string()
});
var repoRoot = mod.object({
  meta: def.cid,
  prev: def.cid.nullable(),
  auth_token: def.cid.nullable(),
  data: def.cid
});
...
var cidCreateOp = mod.object({
  action: mod.literal("create"),
  collection: mod.string(),
  rkey: mod.string(),
  cid: def.cid
});
var cidUpdateOp = mod.object({
  action: mod.literal("update"),
  collection: mod.string(),
  rkey: mod.string(),
  cid: def.cid
});
var deleteOp = mod.object({
  action: mod.literal("delete"),
  collection: mod.string(),
  rkey: mod.string()
});
var cidWriteOp = mod.union([cidCreateOp, cidUpdateOp, deleteOp]);
var recordCreateOp = mod.object({
  action: mod.literal("create"),
  collection: mod.string(),
  rkey: mod.string(),
  value: mod.any()
});
var recordUpdateOp = mod.object({
  action: mod.literal("update"),
  collection: mod.string(),
  rkey: mod.string(),
  value: mod.any()
});
var recordWriteOp = mod.union([recordCreateOp, recordUpdateOp, deleteOp]);
var def2 = {
  ...def,
  repoMeta,
  repoRoot,
...
var leadingZerosOnHash = async (key, fanout) => {
  if ([2, 8, 16, 32, 64].indexOf(fanout) < 0) {
    throw new Error(`Not a valid fanout: ${fanout}`);
  }
  const base3 = `base${fanout}`;
  const zeroChar = toString3(new Uint8Array(1), base3)[0];
  const hash = await sha2562(key);
  const encoded = toString3(hash, base3);
  let count = 0;
...
var layerForEntries = async (entries, fanout) => {
...
var deserializeNodeData = async (blockstore, data, opts) => {
...
// src/mst/mst.ts
var subTreePointer = mod.nullable(def.cid);
...
var nodeDataDef = mod.object({
  l: subTreePointer,
  e: mod.array(treeEntry)
});
var DEFAULT_MST_FANOUT = 16;
var MST = class {
  constructor(blockstore, fanout, pointer, entries, layer) {
  ...
  static async create(blockstore, entries = [], opts) {
  ...
  static async fromData(blockstore, data, opts) {
  ...
  static load(blockstore, cid2, opts) {
    const { layer = null, fanout = DEFAULT_MST_FANOUT } = opts || {};
    return new MST(blockstore, fanout, cid2, null, layer);
  }
  ...
  async getEntries() {
    ...
    if (this.pointer) {
      ...
      const layer = firstLeaf !== void 0 ? await leadingZerosOnHash(firstLeaf.k, this.fanout) : void 0;
  ...
  async save() {
    ...
    const entries = await this.getEntries();
    const data = serializeNodeData(entries);
    await this.blockstore.put(data);
    for (const entry of entries) {
      if (entry.isTree()) {
        await entry.save();
      }
  ...
  async diff(other) {
    ...
    const leftWalker = new walker_default(this);
    const rightWalker = new walker_default(other);
...
// src/util.ts
var ucanForOperation = async (prevData, newData, rootDid, authStore) => {
  ...
  return encode13(ucanForOp);
};
var getCommitPath = async (blockstore, earliest, latest) => {
  let curr = latest;
  const path = [];
  while (curr !== null) {
    path.push(curr);
    const commit2 = await blockstore.get(curr, def2.commit);
    if (earliest && curr.equals(earliest)) {
      return path.reverse();
    }
    const root = await blockstore.get(commit2.root, def2.repoRoot);
...
var RepoStructure = class {
  ...
  static async create(blockstore, did, authStore) {
    let tokenCid = null;
    if (!await authStore.canSignForDid(did)) {
      const foundUcan = await authStore.findUcan(maintenanceCap(did));
      if (foundUcan === null) {
        throw new Error(`No valid Ucan for creating repo`);
      }
      tokenCid = await blockstore.put(encode13(foundUcan));
    }
    ...
    const cid2 = await blockstore.put(commit2);
    logger_default2.info({ did }, `created repo`);
  ...
  static async load(blockstore, cid2) {
    const commit2 = await blockstore.get(cid2, def2.commit);
    const root = await blockstore.get(commit2.root, def2.repoRoot);
    const meta = await blockstore.get(root.meta, def2.repoMeta);
    const data = await MST.load(blockstore, root.data);
    logger_default2.info({ did: meta.did }, "loaded repo for");
    return new RepoStructure({
      blockstore,
      data,
      commit: commit2,
  ...
  async getRecord(collection, rkey) {
    ...
    if (!cid2)
      return null;
    return this.blockstore.getUnchecked(cid2);
  }
  stageUpdate(write) {
  ...
  async createCommit(authStore, performUpdate) {
    let data = this.data;
    ...
    const token = await authStore.canSignForDid(this.did()) ? null : await ucanForOperation(this.data, data, this.did(), authStore);
    ...
    const commitCid = await this.blockstore.put(commit2);
    if (performUpdate) {
  ...
  async revert(count) {
    ...
    for (let i = 0; i < count; i++) {
      const commit2 = await this.blockstore.get(revertTo, def2.commit);
      const root = await this.blockstore.get(commit2.root, def2.repoRoot);
      if (root.prev === null) {
  ...
  async getCarNoHistory() {
    return this.openCar((car) => {
  ...
  async openCar(fn) {
    const { writer, out } = CarWriter2.create([this.cid]);
    try {
      await fn(writer);
    } finally {
      writer.close();
  ...
  async writeCheckoutToCarStream(car) {
    const commit2 = await this.blockstore.get(this.cid, def2.commit);
    const root = await this.blockstore.get(commit2.root, def2.repoRoot);
    await this.blockstore.addToCar(car, this.cid);
    await this.blockstore.addToCar(car, commit2.root);
  ...
  async writeCommitsToCarStream(car, oldestCommit, recentCommit) {
    const commitPath = await getCommitPath(
      this.blockstore,
      oldestCommit,
      recentCommit
    );
    ...
    let prevHead = firstHeadInPath.root.prev !== null ? await RepoStructure.load(this.blockstore, firstHeadInPath.root.prev) : null;
    for (const commit2 of commitPath) {
      const nextHead = await RepoStructure.load(this.blockstore, commit2);
      ...
      if (prevHead === null) {
        ...
        await Promise.all(
          diff.newCidList().map((cid2) => this.blockstore.addToCar(car, cid2))
        );
      }
      prevHead = nextHead;
    }
...
var structure_default = RepoStructure;

// src/verify.ts
var verifyUpdates = async (blockstore, earliest, latest, verifier) => {
  const commitPath = await getCommitPath(blockstore, earliest, latest);
  if (commitPath === null) {
    throw new Error("Could not find shared history");
  }
  ...
  for (const commit2 of commitPath.slice(1)) {
    const nextRepo = await RepoStructure.load(blockstore, commit2);
...
var Repo = class {
  constructor(params) {
    ...
    this.verifier = params.verifier ?? new Verifier();
  }
  static async create(blockstore, did, authStore, verifier) {
    const structure = await structure_default.create(blockstore, did, authStore);
    logger_default2.info({ did }, `created repo`);
    return new Repo({
      blockstore,
      structure,
  ...
  static async load(blockstore, cid2, verifier, authStore) {
    const structure = await structure_default.load(blockstore, cid2);
    logger_default2.info({ did: structure.meta.did }, "loaded repo for");
    return new Repo({
      blockstore,
      structure,
      authStore,
  ...
  static async fromCarFile(buf2, blockstore, verifier, verifyAuthority = true, authStore) {
    const car = await CarReader2.fromBytes(buf2);
    const roots = await car.getRoots();
    if (roots.length !== 1) {
      throw new Error(`Expected one root, got ${roots.length}`);
    ...
    if (verifyAuthority && verifier) {
      await verifyUpdates(blockstore, null, repo.cid, verifier);
    }
    return repo;
  }
  get did() {
  ...
  getCollection(name3) {
    if (name3.length > 256) {
      throw new Error(
        `Collection names may not be longer than 256 chars: ${name3}`
      );
    }
    return new collection_default(this, name3);
  }
  async safeCommit(write) {
    if (!this.authStore) {
  ...
  async loadRoot(newRoot) {
    this.structure = await structure_default.load(this.blockstore, newRoot);
  }
  async verifyUpdates(earliest, latest) {
  ...
  async loadAndVerifyDiff(buf2) {
    const root = await this.blockstore.loadCar(buf2);
    const diff = await this.verifyUpdates(this.cid, root);
    await this.loadRoot(root);
    return diff;
  }
  async loadCarRoot(buf2) {
    const root = await this.blockstore.loadCar(buf2);
    await this.loadRoot(root);
  }
  async getCarNoHistory() {
